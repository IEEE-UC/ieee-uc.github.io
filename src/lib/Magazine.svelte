<script>
  ...
  onMount(async () => {
    const response = await fetch(
      "/src/editable-content/magazines/magazines.json",
    );
    const data = await response.json();
    magazines = data.magazines.sort((a, b) => b.year - a.year); // Sort newest to oldest
    interestForm = data.interest_form;
  });
</script>

<h1>IEEE Student Magazine</h1>

{#if interestForm.title}
  <div
    style="margin-bottom: 2rem; text-align: center; background-color: #f0f8ff; padding: 1.5rem; border-radius: 8px;"
  >
    <h2 style="font-size: 1.5rem; font-weight: bold;">{interestForm.title}</h2>
    <p
      style="font-size: 1rem; font-weight: normal; max-width: 800px; margin: 1rem auto;"
    >
      {interestForm.description}
    </p>
    <a href={interestForm.link} target="_blank" class="action-button"
      >Interest Form</a
    >
  </div>
{/if}
  ...
  {#each magazines as magazine, index}
    {#if index === 0}
      ...
      >
        ...
        <a
          href={`/src/editable-content/magazines/pdfs/${magazine.year}.pdf`}
          target="_blank"
        >
          <img
    ...
    {:else}
      ...
      >
        ...
        <div>
          ...
          <p style="margin: 0.5rem 0 0; font-size: 1rem; font-weight: normal;">
            {magazine.date}
          </p>
          <p>
            <a
